...
TickType_t BMPTest(TFT_t * dev, char * file, int width, int height) {
	...
	if((result->dib.depth == 24) && (result->dib.compress_type == 0)) {
		...
		ESP_LOGD(__FUNCTION__,"width=%d w=%d _x=%d _w=%d _cols=%d _cole=%d", width, w, _x, _w, _cols, _cole);
		...
		ESP_LOGD(__FUNCTION__,"height=%d h=%d _y=%d _rows=%d _rowe=%d", height, h, _y, _rows, _rowe);
...
TickType_t JPEGTest(TFT_t * dev, char * file, int width, int height) {
	...
	ESP_LOGI(__FUNCTION__, "decode_jpeg err=%d imageWidth=%d imageHeight=%d", err, imageWidth, imageHeight);
	if (err == ESP_OK) {
...
void SSD1283(void *pvParameters)
{
	// set font file
	FontxFile fx16G[2];
	FontxFile fx16L[2];
	...
	InitFontx(fx16L,"/spiffs/LATIN16B.FNT",""); // 8x16Dot Latin
	...
	while(1) {
		...
		BMPTest(&dev, file, CONFIG_WIDTH, CONFIG_HEIGHT);
		WAIT;
		CodeTest(&dev, fx16G, CONFIG_WIDTH, CONFIG_HEIGHT);
		WAIT;
		CodeTest(&dev, fx16L, CONFIG_WIDTH, CONFIG_HEIGHT);
		WAIT;
	}
#endif
...
void app_main(void)
{
	...
	xTaskCreate(SSD1283, "SSD1283", 1024*6, NULL, 2, NULL);
}
