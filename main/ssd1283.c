...
bool spi_master_write_addr(TFT_t * dev, uint8_t addr1, uint8_t addr2)
{
...
bool spi_master_write_command_data(TFT_t * dev, uint8_t cmd, uint16_t data)
{
	spi_master_write_command(dev, cmd);
	return spi_master_write_data_word(dev, data);
}
...
void lcdInit(TFT_t * dev, int width, int height, int offsetx, int offsety)
{
	...
	spi_master_write_command_data(dev, 0x10, 0x2F8E);
	spi_master_write_command_data(dev, 0x11, 0x000C);
	spi_master_write_command_data(dev, 0x07, 0x0021);
	spi_master_write_command_data(dev, 0x28, 0x0006);
	spi_master_write_command_data(dev, 0x28, 0x0005);
	spi_master_write_command_data(dev, 0x27, 0x057F);
	spi_master_write_command_data(dev, 0x29, 0x89A1);
	spi_master_write_command_data(dev, 0x00, 0x0001);
	delayMS(100);
	spi_master_write_command_data(dev, 0x29, 0x80B0);
	delayMS(30);
	spi_master_write_command_data(dev, 0x29, 0xFFFE);
	spi_master_write_command_data(dev, 0x07, 0x0223);
	delayMS(30);
	spi_master_write_command_data(dev, 0x07, 0x0233);
	spi_master_write_command_data(dev, 0x01, 0x2183);
	spi_master_write_command_data(dev, 0x03, 0x6830);
	spi_master_write_command_data(dev, 0x2F, 0xFFFF);
	spi_master_write_command_data(dev, 0x2C, 0x8000);
	spi_master_write_command_data(dev, 0x27, 0x0570);
	spi_master_write_command_data(dev, 0x02, 0x0300);
	spi_master_write_command_data(dev, 0x0B, 0x580C);
	spi_master_write_command_data(dev, 0x12, 0x0609);
	spi_master_write_command_data(dev, 0x13, 0x3100);
...
void lcdDrawPixel(TFT_t * dev, uint16_t x, uint16_t y, uint16_t color){
	...
	spi_master_write_command(dev, 0x44);	// Horizontal RAM address position
	spi_master_write_addr(dev, _x, _x);
	spi_master_write_command(dev, 0x45);	// Vertical RAM address position
	spi_master_write_addr(dev, _y, _y);
	spi_master_write_command(dev, 0x21);	// RAM address set
	spi_master_write_addr(dev, _y, _x);
	spi_master_write_command(dev, 0x22);	//	Memory Write
...
void lcdDrawMultiPixels(TFT_t * dev, uint16_t x, uint16_t y, uint16_t size, uint16_t * colors) {
	...
	spi_master_write_command(dev, 0x44);	// Horizontal RAM address position
	spi_master_write_addr(dev, _x2, _x1);
	spi_master_write_command(dev, 0x45);	// Vertical RAM address position
	spi_master_write_addr(dev, _y2, _y1);
	spi_master_write_command(dev, 0x21);	// RAM address set
	spi_master_write_addr(dev, _y1, _x1);
	spi_master_write_command(dev, 0x22);	// Write Data to GRAM
...
void lcdDisplayOff(TFT_t * dev) {
	spi_master_write_command_data(dev, 0x00, 0x0000);	//Display off
}
 
// Display ON
void lcdDisplayOn(TFT_t * dev) {
	spi_master_write_command_data(dev, 0x00, 0x0001);	//Display on
}
...
void lcdInversionOff(TFT_t * dev) {
	spi_master_write_command_data(dev, 0x01, 0x2183); //<R><G><B> color is assigned from S0
}

// Display Inversion On
void lcdInversionOn(TFT_t * dev) {
	spi_master_write_command_data(dev, 0x01, 0x2983); //<B><G><R> color is assigned from S0
}
